...
function registerImageRoutes(app, mongoClient) {
    ...
    app.get("/api/images", async (req, res) => {
        try {
            ...
            if (req.query.downloadFilter) {
                const downloadFilter = req.query.downloadFilter;
    ...
    app.get("/api/images/author/:username", async (req, res) => {
        try {
    ...
    app.patch("/api/images/:id/tags", auth_1.verifyAuthToken, async (req, res) => {
        try {
            const imageId = req.params.id;
            const { tags } = req.body;
            const authUsername = res.locals.token.username;
            if (!Array.isArray(tags) || tags.length > 5 || tags.some((tag) => tag.length > 10)) {
    ...
    app.patch("/api/images/:id/download", async (req, res) => {
        try {
            const imageId = req.params.id;
    ...
    app.delete("/api/images/:id", auth_1.verifyAuthToken, async (req, res) => {
        try {
            const authUsername = res.locals.token.username;
            const imageId = req.params.id;
            const image = await imageProvider.getImageById(imageId);
